...
const buildApp = async (): Promise<AppContainer> => {
  const maxAttempts = 3;
...
export const getApp = async (): Promise<AppContainer> => {
  if (!globalForApp.__shiftBotApp) {
    const appPromise = buildApp().catch((error) => {
      if (globalForApp.__shiftBotApp === appPromise) {
        globalForApp.__shiftBotApp = undefined;
      }
      throw error;
    });
    globalForApp.__shiftBotApp = appPromise;
  }
