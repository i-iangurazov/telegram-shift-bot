...
- Оформленные Excel XLSX: сводка и все смены; старые кнопки выгрузки совместимы.
...
- Фото создаёт запрос подтверждения на 10 минут с момента подготовки запроса. Время смены — исходное `message.date`, включая секунды; задержки подтверждения его не меняют.
...
## Вебхуки и тикер
- Вебхук: `POST /api/telegram/webhook/<WEBHOOK_SECRET>` сначала сохраняет update, затем немедленно запускает единый обработчик очереди. Повторная доставка не исполняет обработанный update снова.
...
### Планировщики
Основные расписания находятся в `vercel.json` и активируются отдельно для каждого проекта Vercel Pro. Установите `CRON_SECRET` в production: запросы Vercel Cron приходят методом GET. Обычный тик — каждые 5 минут; время окончания смены всегда расчётное.

GitHub Actions — резерв. Установите отдельные `PUBLIC_BASE_URL_MAIN`, `PUBLIC_BASE_URL_COPY`, `PUBLIC_BASE_URL_DAILY` и соответствующие `INTERNAL_SECRET_MAIN`, `INTERNAL_SECRET_COPY`, `INTERNAL_SECRET_DAILY`. Матрица запускает и проверяет каждый экземпляр. Рассылки и прогрев GitHub доступны вручную; автоматические рассылки выполняет Vercel. GitHub может отключать scheduled workflows после длительной неактивности репозитория; основные Vercel cron от этого не зависят.

Проверка: авторизованный `/api/internal/health` показывает версию, профиль, timezone, очередь и последний тик. `/api/internal/health?check=xlsx` возвращает пример XLSX на вымышленных данных без уведомлений.
...
Подробности исправления, проверки старых данных и откат: [отчёт 10.09.2026](docs/2026-09-10-fix.md).
